"use client";
import Link from "next/link";
import Image from "next/image";

const roles = [
  {
    title: "gHRig Recruiter",
    desc: "Hire top talent faster",
    slug: "recruiter",
    img: "/assets/images/recruiter-card1.png",
  },
  {
    title: "Job Seeker",
    desc: "Find & apply for jobs",
    slug: "job-seeker",
    img: "/assets/images/jobseeker-card.png",
  },
  {
    title: "Trainer",
    desc: "Teach & earn by training",
    slug: "trainer",
    img: "/assets/images/trainer-card.png",
  },
  {
    title: "Referrer",
    desc: "Refer candidates & earn",
    slug: "referrer",
    img: "/assets/images/referrer-card.png",
  },
  {
    title: "Client / Corporate HR",
    desc: "End-to-end hiring solutions",
    slug: "corporate",
    img: "/assets/images/corporate-card.png",
  },
];

export default function GetStartedPage() {
  return (
    <div className="bg-[#F5F0FA] min-h-screen">
      <div className="max-w-6xl mx-auto py-20 px-6 text-center">

        <h1 className="text-4xl font-semibold text-[#3D3D3D]">
          Welcome to gHRig
        </h1>
        <p className="text-lg text-gray-500 mt-2 mb-14">
          Choose your role to continue
        </p>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 justify-items-center">
          {roles.slice(0, 3).map((role) => (
            <RoleCard key={role.slug} role={role} />
          ))}
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-4 gap-10 mt-10 justify-items-center">
          <div />
          <RoleCard role={roles[3]} />
          <RoleCard role={roles[4]} />
          <div />
        </div>

        <p className="text-gray-500 mt-16">
          Select your role to login or register and start your journey with gHRig
        </p>
      </div>
    </div>
  );
}

/* ================= CARD ================= */

function RoleCard({ role }) {
  return (
   <Link
  href={
    role.slug === "recruiter"
      ? "https://portal.ghrig.in/recruiter/log-in"
      : role.slug === "corporate"
      ? "https://portal.ghrig.in/corporate/log-in"
      : "/coming-soon"
  }
  className="
    relative w-full max-w-[320px]
    min-h-[180px] sm:h-[200px]
    rounded-2xl
    bg-[linear-gradient(135deg,#8A2E55,#4B3E6D)]
    text-white overflow-hidden shadow-lg
    hover:-translate-y-1 transition
  "
>

      {/* TEXT */}
      <div className="p-5 sm:p-6 pt-6 sm:pt-16 relative z-10 text-left">
        <h3 className="text-lg font-semibold">{role.title}</h3>
        <p className="text-sm mt-2 opacity-90 max-w-[85%] sm:max-w-[70%]">
          {role.desc}
        </p>
      </div>

      {/* IMAGE */}
      <div
        className="
          absolute bottom-0 right-0
          w-[120px] h-[120px]
          sm:w-[150px] sm:h-[150px]
          lg:w-[175px] lg:h-[175px]
        "
      >
        <Image
          src={role.img}
          alt={role.title}
          fill
          className="object-contain object-bottom"
        />
      </div>
    </Link>
  );
}
